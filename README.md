...
- **Challenge Mode** — a curated campaign of 23 repair tickets that
  ...
  senior-tier levels (11+) have none.
...
Same underlying menu model, three very different presentations:

- **AMI Aptio** — the modern blue/grey UEFI Setup Utility (the
  default for most tickets).
- **Award / Phoenix** — early-2000s cyan/yellow CMOS Setup with a
  double-line frame and a single vertical menu (no tabs).
- **EFI Shell** — a black terminal: type `setvar vmx Enabled`,
  `bcfg boot dump`, `reset cold`, etc. Tickets routed to this skin
  are CLI-only.
...
- `challenges.py`  — 23 hand-written tickets (data only)
- `procedural.py`  — 15 sabotage templates + deterministic shift generator
- `shifts.py`      — shift scoring + high-score persistence
- `shell.py`       — EFI Shell command parser
- `vendors/`       — `ami.py`, `award.py`, `efi.py` — one render module per skin
- `menu_data.py`   — data-driven definitions of all six BIOS pages
